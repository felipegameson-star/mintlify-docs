...
<Tip>
  Os campos recebidos no webhook ficam disponíveis nas ações com o prefixo **`webhook.`** — por exemplo `{{webhook.status}}`, `{{webhook.product}}` e `{{webhook.amount}}`. A lista de sugestões traz apenas `{{webhook.status}}`; as demais você digita na mão e funcionam normalmente. Veja todas em [Usar variáveis nas mensagens](/automacoes/variaveis).
</Tip>
