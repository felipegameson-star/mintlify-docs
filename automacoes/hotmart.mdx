...
Use esses dados nas mensagens com variáveis como `{{name}}`, `{{webhook.product}}` e `{{webhook.amount}}`. Veja [Usar variáveis nas mensagens](/automacoes/variaveis).
...
> **Compra aprovada (Hotmart)** → **Mensagem**: `Parabéns pela compra, {{name}}! Seu acesso ao {{webhook.product}} já está liberado 🎉` → **Ação**: aplicar tag `cliente` + criar card na etapa "Pós-venda" → **Aguardar** 1 dia → **Perguntar**: "Conseguiu acessar tudo certinho?"
